...
static thread_reference_t trp_button_timer = NULL; //thread reference object ... thread_reference_t = thread_t*
...
volatile uint8_t button_test = 0;
...
{           
	...
	button_test = 1;
	chThdResumeI(&trp_button_timer, (msg_t)button_test);  
	chSysUnlockFromISR();
	CH_IRQ_EPILOGUE();
}

/*********************************************************************************
Thread 9 -  button press timer handler - figures out if a button has been pressed
********************************************************************************/ 
THD_FUNCTION(Thread9, arg) {
  ...
  while (true) {
    ...
    msg = chThdSuspendTimeoutS(&trp_button_timer, TIME_INFINITE);
    chSysUnlock();
...
THD_TABLE_BEGIN  
  THD_TABLE_ENTRY(waThread9, NULL, Thread9, NULL) //Button Press Detector
