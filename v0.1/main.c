...
Copyright (C) 2017 David W. Vernooy
...
double pi = 3.1415926;
double seconds_per_tick = 0.000008; //8us per tick with 64 divider in timer1 & 8MHz clock
...
double power_vector[5];
double power_vector_avg;
double K_power = 0.0;
double J_power = 0.0;
double power_ratio_vector[5];
double K_damp_estimator_vector[5];
double K_damp_estimator = 0.0;
double speed_vector[5];
...
void InitTimer1(void) { 
	...
	TCCR2B = 0b00000111;//clk/1024 which is 128us per tick
	TIMSK2 = 0b00000010;//output compare A interrup  
	OCR2A = 80; //sets time of interrupt

	DDRD = 0b00000000;//enable portD as input
	PORTD|=0b11101100; //turn on pullups
...
{ 
	...
	*************************/
	...
	power_ratio_vector[0]= 0.0;
	power_ratio_vector[1]= 0.0;
	power_ratio_vector[2]= 0.0;
	power_ratio_vector[3]= 0.0;
	power_ratio_vector[4]= 0.0;

	K_damp_estimator_vector[0]= 0.0;
	K_damp_estimator_vector[1]= 0.0;
	K_damp_estimator_vector[2]= 0.0;
	K_damp_estimator_vector[3]= 0.0;
	K_damp_estimator_vector[4]= 0.0;
	...
	lcd_goto_xy(1,1);
	invert = 1;
    fprintf_P(&lcd_out,PSTR("  Open Ergo   "));
	invert = 0;
...
if (updatecount % 500 ==0) {
	...
	lcd_goto_xy(1,3);
		...
		power_ratio_vector_avg = (1.0*power_ratio_vector[4]+2.0*power_ratio_vector[3]+4.0*power_ratio_vector[2]+8.0*power_ratio_vector[1]+16.0*power_ratio_vector[0])/31.0;			
...
*/		
	...
	/************ update average power ************/	
		//lcd_goto_xy(1,4);		
		lcd_goto_xy(4,4);		
		power_vector_avg = (1.0*power_vector[4]+2.0*power_vector[3]+4.0*power_vector[2]+8.0*power_vector[1]+16.0*power_vector[0])/31.0;			
		if (stroke < 5) {
	...
	/************ update 500m split ************/	
		...
		speed_vector_avg = (1.0*speed_vector[4]+2.0*speed_vector[3]+4.0*speed_vector[2]+8.0*speed_vector[1]+16.0*speed_vector[0])/31.0;	
